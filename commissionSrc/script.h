...
var lightForm = document.getElementById('light-form');\
var fanForm = document.getElementById('fan-form');\
var emailForm = document.getElementById('amazon-email-form');\
...
var configure_callback = {\
    ...
    error: function(a, b) {\
        ...
        window.alert('The configuration command failed, check that you \
          are still well connected to the devices WiFi hotspot and retry.'); \
...
var connect_callback={\
    ...
    error:function(a,b) { \
        ...
        window.alert('The connect command failed, check that you are still\
          well connected to the devices WiFi hotspot and retry.');\
        connectButton.innerHTML = 'Retry';\
        enableButtons();\
    }\
};\
...
var copy = function () {\
    window.prompt('Copy to clipboard: Ctrl + C, Enter', deviceID);\
};\
